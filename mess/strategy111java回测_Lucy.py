...
def teststrategy(ibench,ibench_indu,weight_df_,title,feetest=0,shiftdtif=False,filterweight=True,indulist=[],plotindu=False,zhongxinghua=False,inbenchif=False,longshort='long',interval=1,testindu=False,accurateif=True,keycol_price='open'):
    ...
    if longshort=='long':
        alpha_market=r_market.sub(ibench.loc[r_market.index],axis=0)
    ...
    #分行业表现 
        ...
        stock_select_num_indu=weight_df_indu.groupby(['trddt','indu_name']).apply(lambda x:((x>0)|(x<0)).sum())
        r_indu,turnover=get_dayreturn(stocklist_df,price_all,weight_df_indu['Weight'].unstack(),feetest,shiftdtif=shiftdtif,filterweight=filterweight,interval=interval,accurateif=accurateif,keycol_price=keycol_price,induif=True)
        r_indu=r_indu.unstack(1)
        if longshort=='long':   
            alpha_indu=r_indu.sub(ibench_indu.loc[r_indu.index],axis=0)
        elif longshort=='short':
            alpha_indu=r_indu.sub(-ibench_indu.loc[r_indu.index],axis=0)
        else:
            alpha_indu=r_indu
        ...
        stat_alpha_indu.loc['stock_select_num']=stock_select_num_indu.unstack(1).mean().values
        
        stat_alpha_indu_year = alpha_indu.apply(lambda xx:xx.groupby(xx.index.year).apply(lambda x:get_statofreturn(x)))   
...
def get_shift_and_refreq_weight(pos,shiftdtif=False,interval=1,firstorlast='last'):
    ...
    if shiftdtif==True:#防止shift过多，
        pos=pos[pos.index<=Dt_trade[-1]]
    ...
    if (isinstance(interval,str)):
        ...
        pos=(pos.reindex(interval)).reindex(pos.index,method='ffill')
...
# SD='2020-01-01';ED='2020-12-14';benchfield='中证500';modelid=10;keycol_price='vwap';vwaptime=10;feetest=0.00075;testindu=False
def main_funtion(SD,ED,benchfield='中证500',modelid=10,keycol_price='open',vwaptime=10,feetest=0.00075,testindu=False):
    #对全局变量进行global声明
    ...
    if readfromsql==True:
        ...
        df = get_port_fromlocal(r'E:\盈科\组合优化\originaldata\多因子LRX选股202001_202012',col_name_dt='trade_dt',col_name_code='s_info_windcode',col_name_w='weight_hold',sheet_name='Sheet1',shiftdt=True,fillw=False,trddt_s=SD,trddt_e=ED)
    ...
    #风险归因------------------------------------------------------------------------------
    result_exposure,result_exposure_mean,result_exposure_min,result_exposure_max=get_exposure(df)
    
    sql.closesql()
    
    return {'回测':result_strategy,'归因':{'result_unit':result_unit,'result_multi':result_multi},'风险':{'result_exposure':result_exposure,'result_exposure_mean':result_exposure_mean,'result_exposure_min':result_exposure_min,'result_exposure_max':result_exposure_max}}


if __name__ == "__main__":
    result=main_funtion('2020-01-01',str(date.today()))